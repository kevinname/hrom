...
)
{
  ...
  case SDC:
    stat = 0;
    /* It is initialized externally, just reads the status.*/
    if (blkGetDriverState(&SDCD0) != BLK_READY)
      stat |= STA_NOINIT;
    if (sdcIsWriteProtected(&SDCD0))
      stat |=  STA_PROTECT;
    return stat;
#endif
  }
...
)
{
  ...
  case SDC:
    if (blkGetDriverState(&SDCD0) != BLK_READY)
      return RES_NOTRDY;
    if (sdcRead(&SDCD0, sector, buff, count))
      return RES_ERROR;
    return RES_OK;
#endif
  }
...
)
{
  ...
  case SDC:
    if (blkGetDriverState(&SDCD0) != BLK_READY)
      return RES_NOTRDY;
    if (sdcWrite(&SDCD0, sector, buff, count))
      return RES_ERROR;
    return RES_OK;
#endif
  }
...
)
{
  ...
  case SDC:
    ...
    case GET_SECTOR_COUNT:
        *((DWORD *)buff) = mmcsdGetCardCapacity(&SDCD0);
        return RES_OK;
    case GET_SECTOR_SIZE:
        *((WORD *)buff) = MMCSD_BLOCK_SIZE;
        return RES_OK;
